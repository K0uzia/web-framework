...
final class DevDashboard
{
    ...
    public function render(string $template, array $data = [], int $status = 200, string $section = ''): Response
    {
        if ($section === '') {
            $section = match (true) {
                ...
                str_starts_with($template, 'client') => 'client',
    ...
    {
        foreach (['overview', 'pages', 'medias', 'site', 'chrome', 'theme', 'client', 'export'] as $key) {
            $active = $section === $key;
    ...
    private function themeCssMarkup(string $assetRoot): string
    {
        if ($this->site === null) {
            return '';
        }

        return $this->site->themeHeadHtml($assetRoot, null, $this->publicCssDir);
    }
}
