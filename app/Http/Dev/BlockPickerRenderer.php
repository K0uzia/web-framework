...
final class BlockPickerRenderer
{
    ...
    public function renderPickerHtml(): string
    {
        ...
        $cards = $this->renderCardsHtml();

        return $filters . '<div class="dev-block-picker__grid" id="dev-block-picker-grid">' . $cards . '</div>';
    }
    ...
    private function renderFiltersHtml(): string
    {
        ...
        $buttons = ['<button type="button" class="dev-block-filter is-active" data-block-filter="all">Tous</button>'];
        foreach ($groups as $group) {
            $label = self::GROUP_LABELS[$group] ?? ucfirst($group);
            $buttons[] = '<button type="button" class="dev-block-filter" data-block-filter="'
                . htmlspecialchars($group, ENT_QUOTES) . '">'
                . htmlspecialchars($label, ENT_QUOTES) . '</button>';
        }

        return '<div class="dev-block-picker__filters" role="toolbar" aria-label="Filtrer les blocs">'
            . implode('', $buttons) . '</div>';
    }
    ...
    private function renderCard(
        ...
        return '<button type="button" class="dev-block-card" data-block-type="'
