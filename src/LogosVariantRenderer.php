...
final class LogosVariantRenderer
{
    ...
    {
        $itemsHtml = self::itemsHtml($content, $variant);
     ...
     * @param array<string, mixed> $content
     */
    private static function itemsHtml(array $content, string $variant): string
    {
        $html = '';
        foreach (self::items($content, $variant) as $index => $item) {
            $html .= self::logoCellHtml($item, $index, $variant);
        }
     ...
     */
    private static function logoCellHtml(array $item, int $index, string $variant): string
    {
        ...
        $img = '<img class="' . $imgClass . '" src="' . $safeUrl . '" alt="' . $safeAlt
            . '" width="128" height="48" loading="lazy" decoding="async" />';
        $inner = '<div class="section-logos__cell--' . $variant . '">' . $img . '</div>';
