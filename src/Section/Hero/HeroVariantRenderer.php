...
final class HeroVariantRenderer
{
    ...
    private static function renderPhoneButtons(array $content): string
    {
        ...
        if (is_array($linkButton)) {
            $label = htmlspecialchars(trim((string) ($linkButton['label'] ?? '')), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $href = htmlspecialchars(trim((string) ($linkButton['href'] ?? '#')), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            if ($label !== '') {
                $class = $outline !== null && $linkButton === $outline
                    ? 'section-hero__button section-hero__button--outline'
                    : 'section-hero__button--link';
                $parts[] = '<a class="' . $class . '" href="' . $href . '">' . $label . '</a>';
            }
        }
