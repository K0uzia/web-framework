...
final class HeroStyle
{
    ...
    {
        return match (BackgroundType::normalize((string) ($content['background_type'] ?? BackgroundType::IMAGE))) {
            BackgroundType::VIDEO => self::renderHeroVideoBackdrop($content),
            BackgroundType::SHADER => self::renderHeroShaderBackdrop($content),
            default => self::renderHeroImageBackdrop($content),
        };
    }
     ...
     * @param array<string, mixed> $content
     */
    public static function renderHeroImageBackdrop(array $content): string
    {
        $imageUrl = SectionAssets::resolve(
        ...
        return '<div class="section-hero__backdrop section-hero__backdrop--image" aria-hidden="true">'
            . '<img class="section-hero__backdrop-img ' . MediaDisplaySettings::imageFitClass($content, 'section-hero__backdrop-img') . '" src="'
     ...
     * @param array<string, mixed> $content
     */
    public static function renderHeroVideoBackdrop(array $content): string
    {
        $videoUrl = trim((string) ($content['background_video_url'] ?? ''));
        if ($videoUrl === '') {
            return '';
        }
        ...
        $backdropClass = 'section-hero__backdrop section-hero__backdrop--video'
            . ($chromeless ? ' section-hero__backdrop--chromeless' : '');
     ...
     * @param array<string, mixed> $content
     */
    public static function resolveShaderColor(array $content, string $shaderId): string
    {
        $presetColor = ShaderLibrary::colorFor($shaderId);
        $custom = trim((string) ($content['background_shader_color'] ?? ''));
        if ($custom === '') {
            return $presetColor;
        }

        return ShaderLibrary::normalizeColor($custom, $presetColor);
    }

    /**
     * @param array<string, mixed> $content
     */
    public static function renderHeroShaderBackdrop(array $content): string
    {
        $shaderId = ShaderLibrary::normalizeId((string) ($content['background_shader_id'] ?? ''));
        $color = self::resolveShaderColor($content, $shaderId);
        $sceneBackground = ShaderLibrary::sceneBackgroundFor($shaderId);

        return '<div class="section-hero__backdrop section-hero__backdrop--shader" aria-hidden="true"'
            . ' data-hero-shader data-shader-id="' . htmlspecialchars($shaderId, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '"'
            . ' data-shader-color="' . htmlspecialchars($color, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '"'
            . ' style="--shader-fallback:' . htmlspecialchars($sceneBackground, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ';">'
            . '<canvas class="section-hero__shader-canvas"></canvas>'
            . '<span class="section-hero__backdrop-overlay"></span>'
            . '</div>';
    }
